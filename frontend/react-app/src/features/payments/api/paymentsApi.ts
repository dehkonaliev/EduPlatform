import { apiClient } from "../../../lib/api/client";
import type { ApiEnvelope } from "../../../lib/api/types";

// Response shape not confirmed against a real payload yet — treated as
// unknown and only used for its envelope message on the toast for now.
export const paymentsApi = {
  /** POST /api/payments/buy-course — for SPECIAL (one-time purchase) courses, pays from wallet balance. */
  buyCourse: async (courseId: string): Promise<string> => {
    const { data } = await apiClient.post<ApiEnvelope<unknown>>("/payments/buy-course", {
      course: courseId,
    });
    return data.message;
  },
};